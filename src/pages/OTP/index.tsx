...
const OTP: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<OTPFormValues> = async () => {
    ...
    try {
      const response = await doOTPVeify({
        otp: enteredOtp,
    ...
    <div className="h-screen bg-[#1E1B3A] text-white flex flex-col">
      ...
      <div className="flex items-center justify-center h-screen">
        <div className="w-[500px] bg-[var(--primary)] rounded-lg border border-[#555597] p-8">
          <h4 className="relative text-center text-3xl  font-bold text-white mb-4 after:content-[''] after:absolute after:left-1/2 after:-translate-x-1/2 after:-bottom-5 after:w-32 after:h-[3px] after:bg-[#F85E00]">
            Verify OTP
          </h4>
          <p className="px-10 pt-4 text-center text-base">
            Enter the six digit code we sent to your email address to verify
            your Deskmate account.
          </p>
          {!isOtpVerified ? (
            <form
              ...
              <div className="text-center flex mt-5 px-2 justify-between items-center w-full">
                ...
                <button
                  ...
                  className={`rounded-lg text-white font-semibold transition ${
                    timeLeft > 0
                      ? "bg-[var(--primary)] hidden"
                      : "hover:text-[var(--weight)]"
                  }`}
                  disabled={timeLeft > 0}
                >
                  Resend OTP
